...
**نکته:** اگر پاسخ درگاه مقداری را که پکیج به آن نیاز دارد نداشته باشد یا مقدار آن نامعتبر باشد، `create()` خطای `InvalidGatewayDataException` می‌دهد. در این حالت هیچ رکورد پرداختی ذخیره نمی‌شود. پاسخ خام درگاه در context این exception قرار دارد.
...
**نکته:** اگر درگاه کد خطا یا پیام خطای نامعتبر بفرستد، فراخوانی با کد خطای داخلی `9400` ناموفق می‌شود و `error()` نام فیلد نامعتبر را نشان می‌دهد.
...
- اگر پاسخ درگاه یا داده‌های callback مقداری را که نتیجه پرداخت را مشخص می‌کند نداشته باشد یا مقدار آن نامعتبر باشد، `verify()` و `reverse()` خطای `InvalidGatewayDataException` می‌دهند. رکورد پرداخت ذخیره‌شده به‌روز نمی‌شود و پاسخ خام درگاه در context این exception قرار دارد.
